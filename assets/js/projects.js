(function(){
  ...
  const searchEl=document.getElementById('proj-query');
  const yoloEl=document.getElementById('yolo-filter');
  const sortEl=document.getElementById('sort-by');

  function render(){
    const q=(searchEl.value||"").toLowerCase();
    const y=yoloEl.value;
    let show=projects.slice();
    if(y!=="all") show=show.filter(p => (p.yolo||"").toLowerCase()===y);
    if(q) show=show.filter(p => p.title.toLowerCase().includes(q) || p.summary.toLowerCase().includes(q) || p.tags.some(t=>t.toLowerCase().includes(q)));
    if(sortEl.value==="az") show.sort((a,b)=>a.title.localeCompare(b.title));
    gridEl.innerHTML = show.map(p => {
      ...
      <article class="rounded-2xl border border-white/10 bg-slate-900/60 hover:bg-slate-900/70 transition overflow-hidden">
        ...
        <div class="p-5">
          ...
          <div class="mt-4 flex flex-wrap gap-2">${(p.metrics||[]).map(m=>window.helpers.chip(`${m.k}: ${m.v}`)).join('')}</div>
          <div class="mt-4"><a class="underline" target="_blank" href="${p.links?.code||'#'}">Code</a></div>
        </div>
      </article>`;
    }).join('');
  }
  ...
  yoloEl.addEventListener('change', render);
